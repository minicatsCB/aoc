...
/**
 * Check if removing a single number from an array makes it "safe".
 * @param {number[]} arr - The array of numbers.
 * @returns {boolean} - True if removing a single number makes it "safe", otherwise false.
 */
export function canBeMadeSafe(arr) {
...
export function countSafeArrays(arrays) {
    return arrays.reduce((count, arr) => {
        if (isSafe(arr) || canBeMadeSafe(arr)) {
            count++;
        }
        return count;
    }, 0);
